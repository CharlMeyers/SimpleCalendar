...
def determine_max_sentence_length(character_width, ellipsis_width):
...
def highlight_today(draw_element, coordinate_x, coordinate_y, text, font):
...
def add_event(draw_element, coordinate_x, coordinate_y, events):
	...
	max_event_length = determine_max_sentence_length(text_width, ellipsis_width)
...
def write_calendar_day(draw_element, coordinate_x, coordinate_y, calendar_day, events):
	if calendar_day == constants.TODAY.day:
		highlight_today(draw_element, coordinate_x, coordinate_y, str(calendar_day), constants.DATE_FONT)
	else:
...
def build_calendar(events):
	calendar_grid = Image.new('1', (width, height), constants.WHITE)
